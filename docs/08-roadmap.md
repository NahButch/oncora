...
- **Spike (DONE — seam proven): pure-Rust SQLite migration.** The `LedgerStore` seam now has **three interchangeable backends** in `crates/oncora-ledger` — in-memory, **C SQLite** (`rusqlite`, `bundled`), and **pure-Rust SQLite** (`turso`, ex-`limbo`) — and an identical conformance test passes against all three (`cargo test -p oncora-ledger --features "sqlite-c sqlite-rust"`). *Remaining strategy:* (1) build and **thoroughly test** the broader system on the **C** SQLite so the seam is exercised at scale; (2) **swap in the pure-Rust engine** behind the same trait and **re-run the full suite** (unit, `proptest`, golden-set replay); (3) **report any incompatibilities upstream** to Turso to help them succeed. *Verify:* `turso` 0.6 is pre-1.0 — confirm feature coverage (e.g. transactions, concurrency, full SQL) before promoting it past the dev ledger.

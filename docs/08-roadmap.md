...
- **Spike (RESOLVED — turso adopted): pure-Rust SQLite.** The `LedgerStore` seam has three interchangeable backends (in-memory, C SQLite `rusqlite`, pure-Rust `turso`); an identical conformance test passes for all three. A cold-start bulk comparison over 14,469 per-document writes settled the choice: **turso completed with 0 errors at mean 2.04 ms — ~23% faster than C SQLite** ([validation](09-validation.md), [comparison](10-cold-compare.md)). **Decision: turso is the chosen embedded ledger**; the ledger is wired into the agent loop (a provenance record per run) and turso is the bench default. C SQLite is kept behind a benchmark-only feature. *Still to verify before production:* turso's transaction/concurrency/full-SQL coverage; Postgres remains the HA path. Knock-on: gating C SQLite removes the native-`sqlite3` `links` clash that was one of cozo's two blockers (the `graph_builder`/`rayon` one remains).

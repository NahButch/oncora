...
| SQLite (via `sqlx`) | **Fallback (dev relational)** | Same `sqlx` code path, single-file DB for the dev profile | Dev/single-node only; production ledger is Postgres for HA. |

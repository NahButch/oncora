...
> **Implemented (oxigraph) + finding on cozo.** `oncora-kg` ships `OxigraphGraphStore` behind the `GraphStore` trait under `--features oxigraph` (pure-Rust in-memory RDF quad store; `default-features = false` keeps RocksDB out). It passes the same conformance test as `InMemoryGraphStore`, with per-edge **confidence** carried in the quad's named-graph component (`urn:oncora:conf:<v>`). **cozo is deferred** for build-integration reasons, not capability. Originally there were two independent blockers: (1) cozo's `minimal` feature pulls native SQLite (`storage-sqlite` → `sqlite3-src`), which **`links`-clashed** with the C-SQLite ledger (`rusqlite`); and (2) the only way to get the in-memory engine without SQLite (`graph-algo`) pulls `graph_builder 0.4.1`, which does not compile against current `rayon`. **Choosing turso for the ledger removes blocker (1)** — with C SQLite gated to a benchmark-only feature, nothing else links native `sqlite3`, so the `links` collision is gone. **Blocker (2) remains**: cozo still needs `rayon` (only reachable via `graph-algo` → `graph_builder 0.4.1`), so it won't compile until that's pinned/patched or fixed upstream. Re-evaluate then, or run cozo in its own process behind the `GraphStore` trait. The seam means the swap costs nothing downstream.
...
| SQLite — **pure Rust** (`turso`, ex-`limbo`) | **CHOSEN (dev relational / ledger)** | The Rust-native embedded relational/ledger backend — no C in the build. Validated at scale: 14,469 sequential per-document writes, **0 errors, mean 2.04 ms — ~23% faster than the C amalgamation** (see [10-cold-compare.md](10-cold-compare.md)). Default for `oncora-ledger` and the bench. | `turso` is young/pre-1.0 — **verify** transactions/concurrency/SQL coverage before depending on it past the dev ledger; Postgres remains the HA production path. |
| SQLite — **C library** (`rusqlite`, `bundled`) | **Benchmark-only (gated)** | Retained behind `--features sqlite-c` solely to reproduce the C-vs-Rust comparison. Not the default; pulls the native `sqlite3` amalgamation. | Kept opt-in so the default build stays pure-Rust and free of the native-`sqlite3` `links` constraint. |
| **C ↔ Rust SQLite** | **Decided: Rust (turso)** | Both backends live in `crates/oncora-ledger` behind one `LedgerStore` trait; an identical conformance test passes for in-memory, C SQLite, and turso (`cargo test -p oncora-ledger --features "sqlite-c sqlite-rust"`). The cold-start bulk comparison settled it in turso's favour. | Default workspace build stays pure-Rust + fast (backends are opt-in Cargo features). |
...
**Decision.** `redb` for embedded KV/log (fallback `fjall`), `sled` rejected for flux; **pure-Rust SQLite (`turso`)** for the embedded dev/relational ledger (validated faster than C SQLite at 14.5k writes), with `sqlx`+Postgres for the HA production provenance ledger; C SQLite (`rusqlite`) retained behind a benchmark-only feature; `foundationdb` optional and phase-gated; `pgvector` optional, never the primary vector store. All implement `MemoryStore` / `LedgerStore` (see [02-memory.md](02-memory.md), [10-cold-compare.md](10-cold-compare.md)).

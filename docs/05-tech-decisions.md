...
| SQLite — **C library** (via `sqlx`/`rusqlite`) | **Fallback (dev relational)** | Same `sqlx` code path, single-file DB for the dev profile. NB: this is the upstream **C** SQLite amalgamation linked into the binary — a non-Rust dependency isolated behind `sqlx` + the relational/`MemoryStore` seam (see §Non-Rust isolation). | Dev/single-node only; production ledger is Postgres for HA. **Planned:** start on the C version and harden the system against it, then swap to a pure-Rust SQLite (Turso's rewrite, ex-`limbo`) behind the same seam — see the SQLite-swap spike in [08-roadmap.md](08-roadmap.md). |

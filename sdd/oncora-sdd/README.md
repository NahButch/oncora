...
This directory holds the **full** Spec-Driven Development artifact set for **Oncora**
(Oncology Reasoning Agents): a Rust-native, on-prem, uncertainty-aware agentic
reasoning platform for oncology drug discovery.

SDD separates the **what/why** (specification) from the **how** (plan, contracts,
data model) and from the **do** (tasks), so that intent is authored, reviewed, and
versioned before implementation, and every line of code traces to a requirement.
These documents are derived losslessly from the Oncora design canon (`docs/00`–`docs/11`)
and reframed into the SDD methodology.

A second, information-equivalent but compact rendering lives in
[`../oncora-sdd2/`](../oncora-sdd2/) — same facts, denser form.
...
Every functional requirement (`FR-*`), non-functional requirement (`NFR-*`), and
principle (`P-*`) carries a stable ID. Plan sections, contracts, tasks, and
quickstart scenarios reference those IDs so the chain
**principle → requirement → design → task → acceptance test** is auditable end to end.

## Source provenance

Authoritative source: the Oncora `docs/` canon at repo HEAD —
`00-overview`, `01-architecture`, `02-memory`, `03-uncertainty-reliability`,
`04-knowledge-and-data`, `05-tech-decisions`, `06-eval-benchmarking`,
`07-repo-layout`, `08-roadmap`, plus the prototype evidence in
`09-validation`, `10-cold-compare`, and `11-status`.
